...
export default function App() {
  ...
  const extractedOrInFlightRef = useRef<Set<number>>(new Set());

  // Preload the 6 scattered collage images during the intro screen so they are instant when entered
  useEffect(() => {
    SCATTERED_IMAGES.forEach((img) => {
      const cacheImg = new Image();
      cacheImg.src = img.url;
    });
  }, []);

  // Lazy-extract colors and preload reason images as the user scrolls
  useEffect(() => {
    // We want to preload and extract colors for:
    // - The current reason (displayActiveId)
    // - The next 2 reasons (for smooth forward scrolling)
    // - The previous reason (for smooth backward scrolling)
    const activeId = displayActiveId;
    const targetIds = [activeId - 1, activeId, activeId + 1, activeId + 2].filter(
      (id) => id >= 1 && id <= 30
    );

    targetIds.forEach((id) => {
      const reason = CURATED_REASONS[id - 1];
      if (!reason) return;

      // 1. Preload the image in browser cache
      const img = new Image();
      img.src = reason.image;

      // 2. Extract color if not already cached and no static override exists
      const hasOverride = reason.bgColorOverride || reason.overrideColor;
      if (!hasOverride && !extractedOrInFlightRef.current.has(id)) {
        extractedOrInFlightRef.current.add(id);
        ...
        <AnimatePresence mode="wait">
          {!started ? (
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 px-8 py-12 z-30 bg-zinc-950 overflow-hidden text-center"
            >
              ...
              {/* Centered layout box */}
              <div className="relative w-full h-full max-w-sm mx-auto flex flex-col justify-between z-10 pointer-events-none">
                ...
                <div className="flex flex-col gap-5 my-auto text-left pointer-events-none select-none">
                ...
                <div className="flex flex-col items-center gap-4 pb-4 w-full pointer-events-auto">
              ...
              {scrollProgress >= 31.5 && (
                <div className="absolute inset-0 flex flex-col justify-center items-center px-6 text-center pointer-events-none z-15 bg-white select-none">
                  ...
                  <div className="flex flex-col items-center w-full max-w-[320px] pointer-events-auto">
                    ...
                    {/* Cake emoji with clean floating animation */}
                    <motion.div
                      animate={{
                        y: [0, -10, 0],
                        scale: [1, 1.03, 1],
                      ...
                      className="mb-8 flex justify-center items-center text-[120px] filter drop-shadow-[0_12px_24px_rgba(0,0,0,0.12)] select-none leading-none"
                    >
                      🎂
                    </motion.div>
